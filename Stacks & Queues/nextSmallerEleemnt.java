...
public class Solution {
    static ArrayList<Integer> nextSmallerElement(ArrayList<Integer> arr, int n) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            // If the top element of the stack is smaller than the current element, then it
            // is the next smaller element
            else if (stack.peek() < current) {
                result.add(stack.peek());
            }
            // If the top element of the stack is greater than or equal to the current
            // element, then pop elements from the stack until a smaller element is found or
